...
let test name (f: Dual<float> -> Dual<float>) (df_analytical: float -> float) x =
...
// Power
test "Pow Dual-Dual"  (fun x -> Dual<float>.Pow(x, x)) (fun x -> (x ** x) * (log x + 1.0)) x0
test "Pow Dual-Float" (fun x -> Dual<float>.Pow(x, 3.0)) (fun x -> 3.0 * (x ** 2.0)) x0
test "Pow Float-Dual" (fun x -> Dual<float>.Pow(3.0, x)) (fun x -> (3.0 ** x) * log 3.0) x0
...
test "Sqrt" (fun x -> Dual<float>.Sqrt(x)) (fun x -> 0.5 / sqrt x) x0
...
let g (T: Dual<float>) = 10.0 + 2.5 * T - 3.0 * T * log T + 0.5 * T ** 2.0
