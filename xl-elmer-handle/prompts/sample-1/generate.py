...
# Add CLR reference to the assembly
clr.AddReference(str(dll_dir / "Xl.Elmer.Sif.dll"))
...
def main():
    ...
    print("Creating SifDocument...")
    sif = SifDocument()

    # ────────────────────────────────────────────────────────────────────────────
    # Header — Mesh DB, Include Path, and Results Directory use special
    # SIF directives (no '=').
    # ────────────────────────────────────────────────────────────────────────────
    ...
    # ────────────────────────────────────────────────────────────────────────────
    # MATC script variables — written as "$ name = expr;" before the Simulation
    # section.
    # ────────────────────────────────────────────────────────────────────────────
    print("Adding MATC scripts...")
    ...
    molten.Name = "Molten"
    molten.Density = MaterialPropertyValue.IncludeFile("Temperature", "data/rho.dat")
    molten.Emissivity = 0.90
    molten.HeatConductivity = MaterialPropertyValue.IncludeFile(
        "Temperature", "data/k.dat"
    )
    molten.HeatCapacity = MaterialPropertyValue.IncludeFile("Temperature", "data/cp.dat")
    ...
    molten.PoissonRatio = 0.25
    molten.HeatExpansionCoefficient = MaterialPropertyValue.IncludeFile(
        "Temperature", "data/alpha.dat"
    )
    molten.ReferenceTemperature = MaterialPropertyValue.Raw("$(T_MELT)")
    molten.InternalEnergy = MaterialPropertyValue.IncludeFile(
        "Temperature", "data/h.dat"
    )

    # ── Material 2: Dummy (gap) ───────────────────────────────────────────────────
    dummy = sif.AddMaterial()
    dummy.Name = "Dummy"
